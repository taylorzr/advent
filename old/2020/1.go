...
func main() {
	...
	fmt.Printf("Combos of 2\n")
	recur(inputs, 2, []int{}, 0)

	fmt.Printf("Combos of 3\n")
	recur(inputs, 3, []int{}, 0)
...
func recur(inputs []int, length int, cur []int, pos int) {
	if length == 0 {
		// fmt.Printf("%v\n", cur)
		sum := 0
		mul := 1
	...
	for i := pos; i < len(inputs); i++ {
		cur = append(cur, inputs[i])
		recur(inputs, length-1, cur, i+1)
		cur = cur[:len(cur)-1]
	}
}
